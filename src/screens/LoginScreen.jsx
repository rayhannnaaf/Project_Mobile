...
export default function LoginScreen({ navigation, onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // data.user is available if needed
      if (onLogin) onLogin();
    } catch (err) {
      const msg = err?.message ?? 'Login gagal. Periksa email dan password.';
      Alert.alert('❌ Login Gagal', String(msg));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleForgotPassword = async () => {
    if (!email.trim()) {
      Alert.alert('⚠️ Masukkan Email', 'Isi kolom email terlebih dahulu, lalu tekan "Lupa password?".');
      return;
    }
    try {
      const { error } = await supabase.auth.resetPasswordForEmail(email.trim());
      if (error) throw error;
      Alert.alert('📧 Email Terkirim', 'Link reset password sudah dikirim ke ' + email.trim());
    } catch (err) {
      Alert.alert('❌ Gagal', String(err?.message ?? 'Gagal mengirim email reset.'));
    }
  };
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        >
          ...
          <Animated.View style={[s.card, entrance(formAnim, 50)]}>
            ...
            <SpringButton onPress={handleLogin} style={s.loginBtn} disabled={loading === true}>
              <Text style={s.loginBtnTxt}>{loading === true ? '⏳ Memproses...' : 'Masuk →'}</Text>
            </SpringButton>
            ...
            {/* Social buttons — wire up Supabase OAuth when needed */}
            <View style={s.socialRow}>
              <SpringButton
                style={s.socialBtn}
                onPress={() => Alert.alert('Google Login', 'Tambahkan supabase.auth.signInWithOAuth({ provider: "google" })')}
              >
                <Text style={s.socialTxt}>{'Google'}</Text>
              </SpringButton>
              <SpringButton
                style={s.socialBtn}
                onPress={() => Alert.alert('Apple Login', 'Tambahkan supabase.auth.signInWithOAuth({ provider: "apple" })')}
              >
...
const s = StyleSheet.create({
  ...
  socialBtn:   { flex: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 12, borderRadius: 12, backgroundColor: '#112a47', borderWidth: 1, borderColor: '#1E3A5F' },
